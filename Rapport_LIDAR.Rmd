...
La couche obtenu est exporté vers QGis pour être interprétée directement par le gestionnaire. Les cloisonnements sont les lignes fines, régulièrement espacée et parallèles. 
...
Avant d'aboutir à cette méthode, d'autres ont été explorées *(Figure 4)*. Le calcul du MNT permet dans certains cas de faire apparaître les cloisonnements par une faible différence de hauteur *référence des allemands* mais nécessite une précision très importante, un choix d'ombrage bien ajusté et un terrain relativement régulier. Pour améliorer cette méthode, le calcul de la dérivée seconde du MNT (Edge detection) *référence baguette* permet de faire ressortir les variations locales d'altitude telles que des bordures de routes et de cloisonnements. Enfin la méthode retenue dans ce projet prend en compte le nombre de retour et le MNH en plus de l'Edge detection.
...
Il a également été remarqué que ce programme ne fonctionne pas pour détécter les
cloisonnements lorsque la densité de couvert est trop élevée, un autre projet
pour adapter ce programme aux différents contexte forestier pourrait donc être
envisagé.
