...
\
L'étape d'après consiste à télécharger les tuiles de l'IGN correspondant à la zone d'étude. Cette étape s'appuie sur l'utilisation du package jsonlite qui cherche la source de données et extrait le fichier au format .laz. Un API du package `lidR` existait pour récupérer les données mais ce dernier ne fonctionnait pas. La solution apportée permet donc de contourner ce problème, mais les liens n'étant pas pérennes le risque que ceux-ci expirent subsiste.
...
Ensuite, deux séquences sont créées : `x` et `y`. Ces séquences sont générées à partir des paramètres d'entrée, avec un pas de 10000 unités. Cela permet de diviser la zone géographique en une grille de points espacés de 10000 unités, qui seront utilisés pour interroger le service web de données LiDAR.
...
Cette partie met en place deux boucles imbriquées qui vont itérer sur toutes les combinaisons possibles de coordonnées `x` et `y`. Chaque itération représente un point spécifique de la grille créée précédemment. Ces boucles permettent de parcourir systématiquement l'ensemble de la zone géographique définie.
